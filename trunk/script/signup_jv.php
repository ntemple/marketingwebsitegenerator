...
$q3=new Cdb;
	...
	if (get_setting("enable_jv")!=1){
		...
		$t->set_file("content", "signup_jv.html");
	...
	if ($jv_error==1) $error.="<center><b>You must enter the code you received</b></center><br>";
	else if ($jv_error==2) $error.="<center><b>The code provided for signup is wrong</b></center><br>";
...
if ($_GET['captcha']==1) { ?><script language="Javascript">alert("Captcha code incorrectly entered");</script><?php }
	// ConfigArray
...
if (document.form1.password.value == ''){
	...
	err += 'Passwords do not match.\\n';
		...
		"));
		$t->set_var("promo", "Enter the JV code: &nbsp;</td><td> <input type=text size='29' name='jv_code'>");
...
'.($switch ? "<table style='display:none'>{signup_list_b}<tr><td align=\"right\">{description_country_b}: &nbsp;</td><td>{shiping_country_list_b}{required_country_b}</td></tr>{promo1}{code_signup1}{chk}{captcha1}</table>" : "").'</form>
			...
			while ($q->next_record())
			{
				...
				if ((get_setting("enable_oto_paid_signup")==1 && get_setting("free_signup")!=1) || get_setting("give_oto_jv_new")!=1){
				...
				if ($switch){
					...
					$t->set_var("arp_action", "do.signup_jv.php");
					...
					$t->set_var("form_switch","do.signup_jv.php");
...
if (document.form1.".$q->f("field").".value == '0'){
	err += 'No ".$q->f("description").".\\n';
		...
		while ($q->next_record())
		{
			if (get_setting("cut_signup")){
				if ($q->f("field") == 'email'){
					if ($switch) $t->set_var("field_name", "email");
					$t->set_var("field_type", "text");
					$t->set_var("fieldvalue", stripslashes($_GET[$q->f("field")]));
					$t->parse("signup_list_b", "signuplist_b", true);
				}elseif ($q->f("field") == 'first_name'){
					if ($switch) $t->set_var("field_name", "first_name");
					$t->set_var("field_type", "text");
				...
				if (get_setting("signup_with_code")==1)
					{
						$t->set_var("code_signup1", '<input name="signup_code" size="29" type="text">');
					}
					$t->set_var("promo1", "Enter the JV code: &nbsp;</td><td> <input type=text name='jv_code'>");
