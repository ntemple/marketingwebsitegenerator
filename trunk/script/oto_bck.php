<?php
	...
	$q3=new cdb;
	$q2=new cdb;
	$q->query("SELECT * FROM session WHERE secret_pay_id='".md5(get_setting('secret_string').getenv('REMOTE_ADDR'))."' order by id desc limit 1");
	$q->next_record();
if (get_setting("enable_oto_paid_signup") == 1 && get_setting("free_signup")!=1 && get_setting('oto_backup') == 1){
		...
		if (get_setting("enable_oto_1")==1 && get_setting('oto_backup') == 1 && $q->f("paid")==$spaid1 && $q->f("paid_step2")==$spaid2)
	
			{
				$t->set_file("content", "oto_bck.html");
				...
				$query="select * from products where nid='OTO_BCK'";
	...
	if (get_setting('activate_cycler')){
		if ($_COOKIE['cycle']=='')
		{
			$query="select * from cycle WHERE file='oto_bck.php' group by cycle";
			$q->query($query);
			$i=0;
			while ($q->next_record())
			{
				...
				$query="select * from cycle where file='oto_bck.php' AND cycle='".$q->f("cycle")."' order by rand() limit 1";
				$q2->query($query);
				$q2->next_record();
				if ($i==0)
				{
			...
			if ($q->nf() != 0)
				$q2->query("INSERT INTO cycle_stats SET value='".$cookiecycle."', page='oto_bck'");
		...
		else
		{
			...
			$cycle=explode(":", $text);
				...
				$query="select id,cycle,text from cycle WHERE file='oto_bck.php' group by cycle";
				$q3->query($query);
	...
	//END CYCLER CODE
		...
		$occ_buttons=explode("{OTO_BCK}", "".$ocontent."");
	...
	die ($rez_button);
		...
		$member_id=$q->f("id");	
		$seen=$q->f("seen");
		if (get_setting("enable_oto_1")==1 && $seen !=2)
	...
	if (get_setting('activate_cycler')){
			...
			$cycle=explode(":", $text);
				...
				if ($q->f('file') != 'oto_bck.php'){
					...
					if ( preg_match('/('.$q->f("cycle").')\:(\d+)/',$winner,$match)){
						...
						$query="select * from cycle where file='oto_bck.php' AND cycle='".$q->f("cycle")."' AND display=0 order by id limit 1";
						$q2->query($query);
						$q2->next_record();
						if ($q2->nf()){
		...
		//END CYCLER CODE
				replace_tags_t($member_id, $t);
				$query="update members set seen=2 where id='$member_id'";
				$q->query($query);
