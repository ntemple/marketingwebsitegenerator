...
\" id=\"country\" style=\"width: 15em;\">";
	...
	$return_url_stormpay=get_setting("site_full_url")."pay.return.php?s=".$custom.$st2;
	$notify_url_stormpay=get_setting("site_full_url")."pay.ipn.stormpay.php";
	...
	// paypal and stormpay emails will be default as webmaster's
	$paypal_email=get_setting("paypal_email");
	$stormpay_email=get_setting("stormpay_email");
	$aff_flag=false;
	...
	$aff_stormpay_email=$q->f("stormpay_email");
	$jv = $q->f("jv");
	// END NEW
	if ($step2!=1)
	{
	...
	if ($step2==1)
	{
		...
		if (get_setting("splitoption")==2 && $aff_paypal_email)
		{
			...
			$stormpay_email=$aff_stormpay_email;
			$p_stormpay=$stormpay_email;
		}
		else
		{
			$p_paypal=get_setting("paypal_email");
			$p_stormpay=get_setting("stormpay_email");
		}
		if (get_setting("accept_paypal") && trim($p_paypal)!="")
		{
			FFileRead("templates/pay.paypal.html", $pay_paypal_t);
		}
		if (get_setting("accept_stormpay") && trim($p_stormpay)!="")
		{
			FFileRead("templates/pay.stormpay.html", $pay_stormpay_t);
		}
		if (get_setting("splitoption")==2){
			$query="select * from levels where product_id='$product_id' and membership_id='$aff_membership_id' and level=1 order by id asc limit 0,1";
		}else {
	...
	else
	{
		...
		if ($aff_id!="")
		{
			$query="select membership_id, paypal_email, stormpay_email from members where id='".$aff_id."'";
			$q->query($query);
			if ($q->nf()!=0)
			{
				if (get_setting("splitoption")==2)
				{
					$q->next_record();
					if ($q->f("paypal_email")=="" && $q->f("stormpay_email")=="")
					{
						$aff_flag=false;
					}
					else
				...
				else
				{
					...
					if ((get_setting("accept_paypal") && $q->f("paypal_email")!="") || (get_setting("accept_stormpay") && $q->f("stormpay_email")!=""))
					{
						if (get_setting("accept_paypal") && $q->f("paypal_email")!="")
						$aff_paypal_email=$q->f("paypal_email");
						if (get_setting("accept_stormpay") && $q->f("stormpay_email")!="")
						$aff_stormpay_email=$q->f("stormpay_email");
					}
					else
					{
		...
		if ($aff_flag)
		{
			...
			if (get_setting("splitoption")==2)
			{
				...
				if (get_setting("accept_stormpay") && trim($p_stormpay)!="") FFileRead("templates/pay.stormpay.html", $pay_stormpay_t);
			}
			else
			{
				$query="select membership_id, paypal_email, stormpay_email from members where id='".$aff_id."'";
				...
				else
				{
					...
					if (get_setting("accept_stormpay")) FFileRead("templates/pay.stormpay.html", $pay_stormpay_t);
				}
			}
	...
	if ($prodpaypal!=0)
	{
		...
		else
		{
			...
			$pay_buttons=str_replace("{stormpay_amount}",'<input type="hidden" name="amount" value="{amount}">',$pay_buttons);
		}
		if ($trial_flag){
		...
		$pay_buttons=str_replace("{stormpay_email}", $stormpay_email, $pay_buttons);
		...
		$pay_buttons=str_replace("{stormpay_notify_url}", $notify_url_stormpay, $pay_buttons);
		$pay_buttons=str_replace("{stormpay_return_url}", $return_url_stormpay, $pay_buttons);
	...
	if (get_setting('accept_stormpay')==1 && $q->f('stormpay')){
		$pay_buttons=$pay_stormpay_t."<br>";
	}
	$query="SELECT * FROM products WHERE id='$product_id'";
	$q->query($query);
	$q->next_record();
