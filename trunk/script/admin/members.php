...
if ($page) { $t->set_var("page", $page); } else { $t->set_var("page", "1"); }
	...
	if (count($membership_search)>0 && $msearch==2) {$from_change_membership=1;}
			...
			$membership_found=0;
				...
				while ($q2->next_record())
				{
					if ($q2->f("id") != $q->f("membership_id"))
						...
						$members_select .= "<option value='".$q2->f("id")."' selected>".$q2->f("name")."</option>";
						$membership_found=1;
					}	
				}
				if($membership_found!=1) {
					
					$members_select .= "<option value='' selected>No Membership</option>";
				}
...
".
	...
	$vars_new=$nextlink."&order=$order&search=$search&sort=$sort&criteria=$criteria$new_membership$new_date";
