<?php
    ...
    include("inc.top.php");
    $q2 = new CDB;
    $t->set_file("content", "admin.products.html");
    $t->set_file("membershiplist", "admin.membership.select.html");
    $t->set_file("membershiplista", "admin.membership.select.html");
    $t->set_file("productlist", "admin.products.item.html");
    $physical_functions = '';
    $chck_funtions = '';
    $query = "select products.* from products, membership where nid!='OTO1' AND nid!='OTO2' AND nid!='OTO_BCK' AND nid!='OTO2_BCK' AND membership.id=products.membership_id and membership.active=1";
    $q->query($query);
    while ($q->next_record()) {
        if ($q->f("paypal") != 1) {
            $query = "update products set physical='0' where id='".$q->f("id")."'";
            $q2->query($query);
        }
    }
    $query = "select products.* from products, membership where nid!='OTO1' AND nid!='OTO2' AND nid!='OTO_BCK' AND nid!='OTO2_BCK' AND membership.id=products.membership_id and membership.active=1";
    $q->query($query);
    while ($q->next_record()) {
        $months_r = $days_r = $years_r = $years = $days = $months = '';
        ...
        if ($q->f("signup") == 1) {
            $t->set_var($q->f("id")."_checked", "checked");
        } else {
            $t->set_var($q->f("id")."_checked", "");
        }
