      ...
      while(list($i, $v) = each($varname)) {
        ...
        $str = $this->subst($v);
         if ($append) {
			...
			{
			  $this->set_var($target, stripslashes($this->get_var($target) . $str));
			}
			else
			{
			  $this->set_var($target, $this->get_var($target) . $str);
		  ...
		  if (get_magic_quotes_gpc()==0)
			{
			  $this->set_var($target, stripslashes($str));
			}
			else
			{
			  $this->set_var($target, $str);
...
$str = ob_get_clean();
  ...
  * @see       parse
  */
  function pparse2($target, $varname, $append = false) {
    if ($this->debug & 4) {
      echo "<p><b>pparse:</b> passing parameters to parse...</p>\n";
    }
    return $this->finish($this->parse($target, $varname, $append));
    //return false;
  }
 /******************************************************************************
