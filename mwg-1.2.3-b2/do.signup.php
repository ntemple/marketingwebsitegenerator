<?php
  ...
  */
  ...
  if(isset($custom)) { // find out where this would have been set. Globals?
    $sesssignup = $custom;
  } else {
  ...
  switch($validate) {
    ...
    case 1:
      ...
      */
      ...
      $q->query("SELECT product_id FROM session WHERE session_id='$custom'"); //@todo Logic error: custom may be null
      ...
      if ($auth_check_result || ( !isset($cardNumber)) || (get_setting('free_signup')==1) ) {			
        ...
        if ($error != "") {
          ...
          if (get_setting('enable_arp') == 1 && get_setting('arp_in_use_type') == 2) {
            mwg_mail(get_setting("arp_email"), email_replace2(get_setting("arp_message_subject"), $member_id), email_replace2(get_setting("arp_message_body"), $member_id), "From: ".$first_name." <".$email.">");
          }
